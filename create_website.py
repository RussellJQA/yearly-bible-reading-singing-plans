...
def markdown_to_html(md_basename, html_basename, title, description):
    ...
    with open(
        GITHUB_PAGES_DIR / f"{html_basename}.html", "w", encoding="utf-8", newline=""
    ) as write_file:
        output = html_template.substitute(
            title=title, description=description, main_html=main_html
        )
